--  Incremental Load for Global Sales
CREATE OR REPLACE PROCEDURE BL_CL.LOAD_SRC_TO_CLEAN_GLOBAL()
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    WHERE table_name = 'BL_CL.CLEAN_GLOBAL_SALES'
    FOR UPDATE;

    --  Insert Only New `order_dt` Values, Avoiding Duplicate `ORDER_SRC_ID`
    WITH inserted_data AS (
        INSERT INTO BL_CL.CLEAN_GLOBAL_SALES
    ...
    IF v_inserted_count > 0 THEN
        SELECT MAX(order_dt) INTO v_new_max_date FROM BL_CL.CLEAN_GLOBAL_SALES;
    ELSE
