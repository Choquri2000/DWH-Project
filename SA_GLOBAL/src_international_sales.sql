--  Source Table for International Sales
CREATE TABLE IF NOT EXISTS sa_global.src_international_sales (
...
CREATE OR REPLACE PROCEDURE sa_global.incremental_load_international_sales()
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    SELECT COUNT(*) = 0 INTO v_table_empty FROM sa_global.src_international_sales;

    --  If the table is empty, set last_order_date to the **earliest order_dt in external table**
    IF v_table_empty THEN
        SELECT COALESCE(MIN(order_dt), '1900-01-01') INTO v_last_order_date
        FROM sa_global.ext_international_sales;
    ELSE
        --  Otherwise, use the **last loaded order_dt from the target table**
        SELECT COALESCE(MAX(order_dt), '1900-01-01') INTO v_last_order_date
        FROM sa_global.src_international_sales;
    END IF;

    --  Debugging: Show the last order date
    RAISE NOTICE 'Last order date (Global): %', v_last_order_date;

    --  Insert New Records, Ensuring First Load Works Properly
    INSERT INTO sa_global.src_international_sales (
        order_src_id, order_dt, country, city, product_src_id, product_category, 
    ...
    SELECT DISTINCT
        e.order_src_id, e.order_dt, e.country, e.region, e.product_src_id, e.product_category, 
    ...
    FROM sa_global.ext_international_sales e
    WHERE e.order_dt >= v_last_order_date  -- ✅ Ensures first-time load works
    ...
    RAISE NOTICE 'Inserted rows (Global): %', v_inserted_count;

    --  Update last order date only if new rows were inserted
    IF v_inserted_count > 0 THEN
        SELECT MAX(order_dt) INTO v_new_max_date FROM sa_global.src_international_sales;
    ELSE
    ...
    VALUES ('sa_global.src_international_sales', v_new_max_date, v_inserted_count, 'SUCCESS', NULL)
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        WHERE table_name = 'sa_global.src_international_sales';
        RAISE;
END;
$$;
